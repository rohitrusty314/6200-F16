...
public class HW4 {
	...
	String path = "C:/Users/Himanshu/Desktop/IRH4/queries.txt";
	...
	try {
		 ...
		 {
			String tempquery[]  = line.split("\\s+");
			queryID.add(tempquery[0]);
			String curquery = "";
			for(int i = 1 ; i < tempquery.length ;i++)
			{
			  
			  curquery = curquery.concat(tempquery[i]);
	...
	for(String  s1 : queries)
	{
		...
		try {
			...
			finalqueryres = new FileWriter("query_" + queryID.get(ind) + "_LUCENE_cosine_table.txt");
		...
		for (int i = 0; i < hits.length; ++i) {
		    ...
		    System.out.println((i + 1) + ". " + d.get("path")
			    + " score=" + hits[i].score);
		    br5.write(queryID.get(ind) + "\t" + "Q0 " + temp + "\t" + (i + 1) + "\t" + hits[i].score + "\t" + "LUCENE" + "\r\n");
		}
		ind++;
